...
async def _asgi_app(scope, receive, send):
    """Minimal ASGI router — avoids Starlette endpoint wrapping issues."""
    if scope["type"] == "lifespan":
        # Handle lifespan events required by uvicorn
        while True:
    ...
    if path == "/sse":
        # SSE connection — client opens this and keeps it open
...
async def main():
    ...
    logging.info(f"[image-restoration] starting (SSE) · mode={mode} workers={_MAX_WORKERS} host={_HOST} port={_PORT}")
    config = uvicorn.Config(_asgi_app, host=_HOST, port=_PORT, log_level="warning")
